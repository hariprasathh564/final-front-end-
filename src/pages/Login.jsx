...
import { AlertCircle, Loader2, Check, Eye, EyeOff, Sparkles, User, KeyRound } from "lucide-react";
...
export default function Login() {
  ...
  return (
    <div 
      className="relative min-h-screen w-full overflow-hidden bg-slate-100 flex items-center justify-center font-sans perspective-2000"
    ...
    >
      ...
      <motion.div
        ...
        className="relative z-20 w-full max-w-[380px] px-4"
      >
        {/* MASCOT CHARACTER */}
        <Mascot 
          focusState={focusState} 
          textLength={focusState === 'email' ? formData.email.length : 0} 
        ...
        >
          ...
          <div className="text-center mb-6 mt-4">
            <h2 className="text-3xl font-black text-slate-800 tracking-tighter">
              Milk<span className="text-blue-500">Soda</span>
            </h2>
          </div>
          ...
          <form onSubmit={submit} className="space-y-4 relative">
            ...
            {/* Liquid Background for Inputs */}
            <motion.div 
              className="absolute bg-blue-100 rounded-2xl -z-10 pointer-events-none"
              initial={false}
              animate={{
                top: focusState === 'email' ? 0 : focusState === 'password' ? 76 : 0,
                height: 64,
                width: "100%",
            ...
            <div className="flex justify-end">
               {/* FIXED LINK HERE: Changed /forgot to /forgot-password */}
               <Link to="/forgot-password" className="text-[10px] font-black text-gray-400 hover:text-blue-500 tracking-widest uppercase hover:scale-105 transition-transform">Forgot?</Link>
            </div>
            ...
            <motion.button
              ...
              className={`
                ...
                ${isSuccess ? "bg-green-500" : "bg-slate-900"} transition-colors duration-500
              `}
            >
...
const Mascot = ({ focusState, textLength, showPassword }) => {
  ...
  useEffect(() => {
    if (focusState === 'email') {
      lookX.set(Math.min(textLength, 25) - 10); // Track typing
      lookY.set(5);
